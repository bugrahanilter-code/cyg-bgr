...
class RegimeConfig(BaseModel):
    ...
    extreme_atr_pct: float = Field(default=8.0, ge=0.5, le=100.0)
...
class MarketRegimeEngine:
    ...
    def annotate(self, frame: pd.DataFrame) -> pd.DataFrame:
        ...
        for row in result.itertuples(index=False):
            ...
            volatility = self._classify_volatility(
                safe_float(row.regime_atr_pct), safe_float(row.regime_vol_rank)
            )
    ...
    def result_at(self, annotated: pd.DataFrame, index: int, warmup: int = 0) -> RegimeResult:
        ...
        volatility = VolatilityRegime(str(row.get("regime_volatility", VolatilityRegime.UNKNOWN.value)))
    ...
    def _classify_volatility(
        self, atr_pct: float | None, rank: float | None
    ) -> VolatilityRegime:
        if atr_pct is None:
        ...
        if rank >= self.config.extreme_volatility_percentile:
            return VolatilityRegime.EXTREME
        if rank >= self.config.high_volatility_percentile:
            return VolatilityRegime.HIGH
        if rank <= self.config.low_volatility_percentile:
